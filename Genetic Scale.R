...
EnhancedVolcano(ressig,
                ...
                pointSize = 4.0,
                subtitle = NULL,
                col = c('#089392FF', '#260F99FF', '#FFB2B2FF', '#F55F7BFF'))



### cross-validation and improvement 
#Load in the data and format for deseq2 analysis
# Load the file without specifying row.names
countdata <- read.table("/Users/rk092/Downloads/GSE139078_main(2).txt", header = TRUE)
# Make row names unique by appending a suffix
rownames(countdata) <- make.unique(as.character(countdata[, 1]))
countdata <- countdata[, -1]  # Remove the column used for row names
head(countdata)
count_matrix <- data.matrix(countdata)
#count_matrix <- count_matrix[,-1]
head(count_matrix)
## condition is the experimental setup so thats why reordering samples would be important to have all samples of one group together
condition <- factor(c(rep("Control", 5), rep("Virus_and_Drug_Treated", 5)))
...
write.csv(as.data.frame(resdata), file="/Users/rk092/Downloads/GSE139078_validation_sig_genes.csv")
...
# Load your data (replace with your file path)
data <- read.csv("/Users/rk092/Downloads/input_main.csv", row.names = 1)  # Replace with your file
# Ensure data contains DE genes and a 'Response' column
#'ADRB2', 'CXCL8', 'GOPC', 'KCNN4', 
# Define DE genes identified by DESeq2
de_genes <- c('LDLR', 'PSMD5', 'TNF')   # Replace with your DE genes
features <- data[, de_genes]
response <- as.factor(data$Response)  # Response: Binary (e.g., 0 = Non-responder, 1 = Responder)
